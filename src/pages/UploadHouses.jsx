...
import { uploadHouses } from '../services/api';
...
export default function UploadHouses() {
  ...
  const handleUpload = async () => {
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        try {
          const houses = results.data.map((row) => ({
          ...
          })).filter(h => h.address && !isNaN(h.latitude) && !isNaN(h.longitude));

          const res = await uploadHouses(houses);
          setResult(res.data);
          setFile(null);
          setPreview([]);
        } catch (err) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h2 className="text-2xl font-bold text-gray-800 mb-2">Upload Houses</h2>
      <p className="text-gray-500 text-sm mb-6">Upload a CSV with house data including coordinates for map placement.</p>

      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-6">
        <p className="text-sm font-medium text-blue-800 mb-2">CSV Format</p>
        <code className="text-xs text-blue-600 bg-blue-100 px-2 py-1 rounded">address,latitude,longitude</code>
      </div>
      ...
      <label className="block border-2 border-dashed border-gray-300 rounded-xl p-8 text-center cursor-pointer hover:border-cyan-400 hover:bg-cyan-50/50 transition-colors mb-4">
        ...
        <p className="text-sm text-gray-600 font-medium">{file ? file.name : 'Click to select CSV file'}</p>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b">
                {Object.keys(preview[0]).map(k => <th key={k} className="text-left px-2 py-1 text-gray-500">{k}</th>)}
              </tr>
            </thead>
            <tbody>
              {preview.map((row, i) => (
      ...
      >
        <Upload size={18} />
        {uploading ? 'Uploading...' : 'Upload Houses'}
      </button>
      ...
      {result && (
        <div className="mt-4 p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-start gap-3">
          ...
          <p className="font-medium text-emerald-800">{result.message} — {result.count} houses created</p>
        </div>
      )}
